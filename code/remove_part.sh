...
root_dir="/ZPOOL/data/projects/rf1-mbme-pilot/script-testing"
...
for subject_dir in "$root_dir"/sub-*; do
    if [ -d "$subject_dir" ]; then
        ...
        rm -f "$subject_dir"/func/sub-*_task-sharedreward_acq-mb*_part-mag_events.tsv

        # Rename files matching the pattern sub-*_task-sharedreward_acq-mb*_part-phase_events.tsv
        for file in "$subject_dir"/func/sub-*_task-sharedreward_acq-mb*_part-phase_events.tsv; do
            if [ -f "$file" ]; then
